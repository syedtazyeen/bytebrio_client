...
import {
  fetchDataAsync,
  fetchUserDataAsync,
} from "../../../app/redux/bytes/latestByteSlice";

export default function LatestBytesListContainer() {
  const dispatch = useDispatch();
  const {
    data: postData,
    loading: postLoading,
    error: postError,
  } = useSelector((state) => state.latestBytes);
  ...
  return (
    <>
      {postsWithUserData.map((item) => (
        ...
        >
          <div style={{ maxWidth: "32rem" }} className="cursor-pointer  w-full max-w-[26rem] p-8 hover:bg-primary_hover rounded-md border border-secondary_accent">
            <UserInfoPallet name={item.user.name} profile_image={item.user.profile_image} isGreenTick={item.user.isGreenTick} />
            <p 
            style={{ whiteSpace: 'pre-wrap' }}
            className="line-clamp-3 overflow-hidden my-4">{item.data.text}</p>
            <img
